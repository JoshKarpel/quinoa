...
def process(path):
    ...
    # expand to include not just "exact matches" but also blobs with nearly the right area
    singleton_blobs_by_area = [
        blob for blob in seed_blobs if int(np.round(blob.area / rough_seed_area)) == 1
    ]
    ...
    for blob in seed_blobs:
        area_ratio = int(np.round(blob.area / seed_area))
        perim_ratio = int(np.round(blob.perimeter / seed_perim))

        if area_ratio == perim_ratio:
